...
- (void) updateWithBatchNode:(CCSpriteBatchNode*)batchnode rect:(CGRect)rect rotated:(BOOL)rotated capInsets:(CGRect)capInsets
{
	...
	//对“默认”叠加模式进行容错判定
	if(scale9Image.texture != nil)
	{
		if(scale9Image.texture.hasPremultipliedAlpha)
		{
			[scale9Image setBlendFunc:(ccBlendFunc){GL_ONE, GL_ONE_MINUS_SRC_ALPHA}];
		}
		else
		{
			[scale9Image setBlendFunc:(ccBlendFunc){GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA}];
		}
	}
